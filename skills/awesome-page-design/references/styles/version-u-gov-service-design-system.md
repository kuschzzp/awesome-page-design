...
> 基于 `gov-service` 的网站视觉风格提示。参考来源：GOV.UK Design System（https://design-system.service.gov.uk/）。本文件用于学习和迁移设计语言，不复制任何品牌页面或商标资产。
...
**Gov Service** 的核心是：政府服务式高对比、粗黑标题、蓝色链接和黄色焦点。
...
| 次级 Surface | `--surface-2` | `#f4f8fb` | 弱背景、按钮、分区 |
| 主文字 | `--text` | `#0b0c0c` | 标题和正文 |
| 辅助文字 | `--muted` | `#484949` | 摘要、时间、说明 |
| 主操作色 | `--accent` | `#1d70b8` | 按钮、链接、重点数字 |
| 深强调色 | `--accent-2` | `#0f385c` | hover、深色强调 |
| 分割线 | `--line` | `#0b0c0c` | 边框和分隔 |
| 弱强调背景 | `--soft` | `#ffdd00` | 标签、选中态、浅色区域 |

## 3. 排版体系

- 字体栈：`Arial, Helvetica, sans-serif`
