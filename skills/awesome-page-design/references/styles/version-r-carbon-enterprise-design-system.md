# Carbon Enterprise 设计系统文档

> 基于 `carbon-enterprise` 的网站视觉风格提示。参考来源：IBM Carbon Design System（https://carbondesignsystem.com/）。本文件用于学习和迁移设计语言，不复制任何品牌页面或商标资产。
...
**Carbon Enterprise** 的核心是：IBM Carbon 式灰阶层级、蓝色操作色、方正网格和数据密度。
...
| 次级 Surface | `--surface-2` | `#e0e0e0` | 弱背景、按钮、分区 |
| 主文字 | `--text` | `#161616` | 标题和正文 |
| 辅助文字 | `--muted` | `#525252` | 摘要、时间、说明 |
| 主操作色 | `--accent` | `#0f62fe` | 按钮、链接、重点数字 |
| 深强调色 | `--accent-2` | `#0043ce` | hover、深色强调 |
| 分割线 | `--line` | `#c6c6c6` | 边框和分隔 |
| 弱强调背景 | `--soft` | `#edf5ff` | 标签、选中态、浅色区域 |

## 3. 排版体系

- 字体栈：`'IBM Plex Sans', 'Helvetica Neue', Arial, sans-serif`
