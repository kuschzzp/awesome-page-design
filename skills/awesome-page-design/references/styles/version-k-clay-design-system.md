# Version K - Claymorphism Design System
...
Claymorphism is a light web UI direction for friendly SaaS, education, wellness, approachable tools. Its visual personality is friendly, tactile, approachable. Reuse its visual language while designing the actual product structure around user goals, content priority, and workflow requirements.
...
The style language is: lavender background, rounded Nunito type, soft inner and outer shadows. Preserve the depth model that defines the style. If the style uses square borders, avoid soft cards. If it uses glow or glass, keep contrast and focus visibility intact.
...
Best suited for friendly SaaS, education, wellness, approachable tools. Avoid using this style when the brand, audience, or product density conflicts with its personality: friendly, tactile, approachable.

## 11. Comparison With Other Styles

Compared with the rest of the library, Claymorphism is defined by lavender background, rounded Nunito type, soft inner and outer shadows. It should feel different from generic neutral dashboards while still remaining usable in production.
