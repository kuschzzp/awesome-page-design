# Atlassian Workbench 设计系统文档

> 基于 `atlassian-workbench` 的网站视觉风格提示。参考来源：Atlassian Design System（https://atlassian.design/）。本文件用于学习和迁移设计语言，不复制任何品牌页面或商标资产。
...
**Atlassian Workbench** 的核心是：协作产品蓝、Lozenge 标签、任务流卡片和团队工作区。
...
| 次级 Surface | `--surface-2` | `#f1f2f4` | 弱背景、按钮、分区 |
| 主文字 | `--text` | `#172b4d` | 标题和正文 |
| 辅助文字 | `--muted` | `#44546f` | 摘要、时间、说明 |
| 主操作色 | `--accent` | `#0c66e4` | 按钮、链接、重点数字 |
| 深强调色 | `--accent-2` | `#0052cc` | hover、深色强调 |
| 分割线 | `--line` | `#dfe1e6` | 边框和分隔 |
| 弱强调背景 | `--soft` | `#e9f2ff` | 标签、选中态、浅色区域 |
...
| 大圆角 | `10px` | Hero、卡片、面板 |
| 小圆角 | `4px` | Logo、标签、按钮 |
| 阴影 | `0 1px 1px rgba(9,30,66,0.12), 0 8px 16px rgba(9,30,66,0.12)` | 主要容器深度 |
