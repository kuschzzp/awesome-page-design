...
> 基于 `fluent-cloud` 的网站视觉风格提示。参考来源：Microsoft Fluent 2（https://fluent2.microsoft.design/）。本文件用于学习和迁移设计语言，不复制任何品牌页面或商标资产。
...
**Fluent Cloud** 的核心是：Windows 11 式中性色、Segoe 字体、命令栏和轻量景深。
...
|---|---|---|---|
| 页面背景 | `--bg` | `#f5f5f5` | 全局背景 |
| 主 Surface | `--surface` | `rgba(255,255,255,0.82)` | 卡片、面板、导航 |
| 次级 Surface | `--surface-2` | `#ffffff` | 弱背景、按钮、分区 |
| 主文字 | `--text` | `#242424` | 标题和正文 |
| 辅助文字 | `--muted` | `#616161` | 摘要、时间、说明 |
| 主操作色 | `--accent` | `#0f6cbd` | 按钮、链接、重点数字 |
| 深强调色 | `--accent-2` | `#115ea3` | hover、深色强调 |
| 分割线 | `--line` | `#d1d1d1` | 边框和分隔 |
| 弱强调背景 | `--soft` | `#ebf3fc` | 标签、选中态、浅色区域 |

## 3. 排版体系

- 字体栈：`'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif`
...
| 大圆角 | `12px` | Hero、卡片、面板 |
| 小圆角 | `6px` | Logo、标签、按钮 |
| 阴影 | `0 8px 24px rgba(0,0,0,0.10)` | 主要容器深度 |
