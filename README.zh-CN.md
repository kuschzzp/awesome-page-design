...
当前 Skill 专注于布局规划、视觉风格选择、UI 质量审查、实现验收、实现打磨和可复用设计规范。
...
4. 在尚未选择风格时，打开或提供预览页。
5. 请你从预览页复制一个匹配任务类型的风格提示词：完整、落地页、仪表盘、管理后台或移动端。
6. 只有当你明确让 Agent 代选时，才由 Agent 给出少量候选方案。
7. 阅读对应风格文档。
8. 设置布局变化度、动效强度和视觉密度。
9. 把选定风格翻译成具体布局结构、CSS 变量、主题 token、组件细节、按钮规则、反馈规则、间距规则、响应式规则、状态规则或组件类。
10. 应用反模板化检查和相近风格防混淆规则。
11. 当涉及真实 UI 代码时，检查语义控件、可访问标签、焦点状态、响应式文本、稳定媒体尺寸、动效降级，以及有用的空/错/加载状态。
12. 根据真实产品重新设计页面结构，而不是复制示例 HTML。
